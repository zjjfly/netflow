...
abstract class TrafficHandler
  ...
  protected def handOff(actor: Wactor.Address,
  ...
  override def channelRead0(ctx: ChannelHandlerContext, msg: DatagramPacket) {
    ...
    if (msg.content().readableBytes() < 4) {
      warn(
        "Unsupported UDP Packet received from " + sender.getAddress.getHostAddress + ":" + sender.getPort)
      return
    }
    ...
    actor onSuccess { actor: Wactor.Address =>
      handOff(actor, sender, msg.content())
    }
    actor onFailure { e: Throwable =>
      debug(e.getMessage)
      warn(
        "Unauthorized Flow received from " + sender.getAddress.getHostAddress + ":" + sender.getPort)
...
object NetFlowHandler extends TrafficHandler {
  def handOff(actor: Wactor.Address,
              sender: InetSocketAddress,
              buf: ByteBuf): Unit = {
