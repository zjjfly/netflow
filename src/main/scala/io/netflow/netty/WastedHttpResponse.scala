...
private[this] object WastedHttpResponse
    extends http.HttpResponder("netflow.io") {
  override def apply(
      status: HttpResponseStatus,
      body: Option[String] = None,
      mime: Option[String] = None,
      keepAlive: Boolean = false,
      headers: Map[String, String] = Map.empty): FullHttpResponse = {
    val resp = super.apply(status, body, mime, keepAlive, headers)
    resp.headers
      .set(HttpHeaders.Names.CACHE_CONTROL, HttpHeaders.Values.NO_CACHE)
    resp
  }
  ...
  def respond(status: HttpResponseStatus,
              ...
              mime: Option[String] = None,
              keepAlive: Boolean = false,
              headers: Map[String, String] = Map.empty): HttpResponse = {
    apply(status, body, mime, keepAlive, headers)
  }

  object OK {
    def apply(): HttpResponse = respond(HttpResponseStatus.OK)
    def apply(resp: String): HttpResponse =
      respond(HttpResponseStatus.OK, Some(resp))
    def apply(jo: JObject): HttpResponse =
      respond(HttpResponseStatus.OK, Some(Serialization.write(jo)))
  }

  object NotFound {
    def apply(): HttpResponse =
      respond(HttpResponseStatus.NOT_FOUND, Some("""{"status":"NOT_FOUND"}"""))
    def apply(message: String) = {
      val json =
        Serialization.write(("status" -> "NOT_FOUND") ~ ("message" -> message))
