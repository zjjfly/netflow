...
class FlowException(msg: String)
    extends Exception(msg)
    with util.control.NoStackTrace

class InvalidFlowVersionException(version: Int)
    extends FlowException("Version " + version)

class IllegalFlowDirectionException(direction: Int, fd: NetFlowData[_])
    extends FlowException("Direction " + direction + ": " + fd.toString)

class IncompleteFlowPacketHeaderException extends FlowException("")

class CorruptFlowPacketException extends FlowException("")

class CorruptFlowTemplateException(template: Int)
    extends FlowException("TemplateId " + template)

class IllegalTemplateIdException(template: Int)
    extends FlowException("TemplateId " + template)
...
class UnableToGetActorException(x: String)
    extends FlowException("Unable to get Actor for " + x)
