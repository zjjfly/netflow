...
package object lib {
  ...
  final val NoBackendDefined = new IllegalArgumentException(
    "No backend is defined") with scala.util.control.NoStackTrace
  ...
  implicit class RichByteBuf(val buf: ByteBuf) extends AnyVal {
    def getInetAddress(template: flows.cflow.Template,
                       field: TemplateFields.Value): Option[InetAddress] = {
    ...
    def getInetAddress(offset: Int, length: Int = 4): InetAddress =
      Try[InetAddress] {
        ...
        val data =
          (1 to length).map(a => buffer.readUnsignedByte.toByte).toArray
    ...
    def getUnsignedInteger(template: flows.cflow.Template,
                           field: TemplateFields.Value): Option[Long] = {
      if (!template.hasField(field)) return None
      Some(
        getUnsignedInteger(template.typeOffset(field), template.typeLen(field)))
    }

    def getUnsignedInteger(template: flows.cflow.Template,
                           field1: TemplateFields.Value,
                           field2: TemplateFields.Value): Option[Long] =
      getUnsignedInteger(template, field1) orElse getUnsignedInteger(template,
                                                                     field2)
  ...
  class InetPrefixSerializer
      extends CustomSerializer[InetPrefix](format =>
        ({
          case JObject(
              JField("prefix", JString(prefix)) :: JField(
                "prefixLen",
                JInt(prefixLen)) :: Nil) =>
  ...
  class DirectionSerializer
      extends CustomSerializer[TrafficType.Value](format =>
        ({
          case JObject(JField("direction", JString(name)) :: Nil) =>
            TrafficType.withName(name)
        }, {
  ...
  class DateTimeSerializer
      extends CustomSerializer[DateTime](format =>
        ({
          case JObject(JField("date", JString(date)) :: Nil) =>
            new DateTime(date)
        }, {
