...
private[netflow] object NodeConfig extends Logger {
  ...
  case class ServerConfig(cores: Int,
                          statuslog: Duration,
  ...
  case class AdminConfig(authKey: UUID, signKey: UUID)

  case class HttpConfig(listen: Seq[InetSocketAddress],
                        sendFile: Boolean,
                        sendBuffer: Boolean,
                        gzip: Boolean,
  ...
  case class CassandraConfig(hosts: Seq[String],
                             keyspace: String,
                             minConns: Int,
                             maxConns: Int,
                             minSimRequests: Int,
  ...
  case class TcpConfig(sendBufferSize: Integer,
  ...
  case class SFlowConfig(listen: Seq[InetSocketAddress], persist: Boolean)

  case class NetFlowConfig(listen: Seq[InetSocketAddress],
                           persist: Boolean,
  ...
  private def load(): ServerConfig = {
    val adminAuthKey = Config
      .getString("admin.authKey")
      .flatMap(ak => Tryo(UUID.fromString(ak))) match {
      case Some(ak) => ak
      case _ =>
        val authKey = UUID.randomUUID()
        error(
          "Invalid or missing UUID at admin.authKey directive. Generated %s",
          authKey)
        authKey
    }

    val adminSignKey = Config
      .getString("admin.signKey")
      .flatMap(sk => Tryo(UUID.fromString(sk))) match {
      case Some(sk) => sk
      case _ =>
        val signKey = UUID.randomUUID()
        error(
          "Invalid or missing UUID at admin.signKey directive. Generated %s",
          signKey)
        signKey
    }

    val admin = AdminConfig(authKey = adminAuthKey, signKey = adminSignKey)
    ...
    val http = HttpConfig(
      ...
      maxContentLength =
        Config.getBytes("http.maxContentLength", 10 * 1024 * 1024),
      maxInitialLineLength =
        Config.getBytes("http.maxInitialLineLength", 8 * 1024),
    ...
    val tcp = TcpConfig(
      sendBufferSize =
        Config.getBytes("http.tcp.sendBufferSize", 10 * 1024 * 1024).toInt,
      receiveBufferSize =
        Config.getBytes("http.tcp.receiveBufferSize", 1024 * 1024).toInt,
    ...
    val storage =
      Config.getString("storage").flatMap(n => Tryo(StorageLayer.withName(n)))
    ...
    info("Using %s of %s available cores",
         server.cores,
