...
object NetFlowV6Packet {
  ...
  /**
    ...
    */
  def apply(sender: InetSocketAddress, buf: ByteBuf): Try[NetFlowV6Packet] =
    Try[NetFlowV6Packet] {
      ...
      val flows: List[NetFlowV6] = (0 to count - 1).toList.flatMap { i =>
        apply(sender,
              buf.slice(headerSize + (i * flowSize), flowSize),
              id,
              uptime,
              timestamp,
              samplingInterval)
  ...
  def apply(sender: InetSocketAddress,
            ...
            uptime: Long,
            timestamp: DateTime,
            samplingInterval: Int): Option[NetFlowV6] =
    Try[NetFlowV6] {
      ...
      NetFlowV6(
        ...
        Some(buf.getUnsignedInteger(24, 4))
          .filter(_ != 0)
          .map(x => timestamp.minus(uptime - x)), // start
        Some(buf.getUnsignedInteger(28, 4))
          .filter(_ != 0)
          .map(x => timestamp.minus(uptime - x)), // stop
        buf.getInetAddress(0, 4), // srcAddress
        buf.getInetAddress(4, 4), // dstAddress
        Option(buf.getInetAddress(8, 4))
          .filter(_.getHostAddress != "0.0.0.0"), // nextHop
  ...
  private def doLayer[T](
      f: FlowPacketMeta[NetFlowV6Packet] => Future[T]): Future[T] =
