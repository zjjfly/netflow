...
private[netflow] abstract class TemplateMeta[T <: Template](
    implicit
    m: Manifest[T]) {
  def apply(sender: InetSocketAddress,
            ...
            fpId: UUID,
            flowsetId: Int,
            timestamp: DateTime): Try[T] = Try[T] {
    ...
    flowsetId match {
      case 0 | 2 =>
        ...
        while (idx < fieldCount) {
          ...
          map ++= Map("offset_" + typeName -> dataFlowSetOffset,
                      "length_" + typeName -> typeLen)
...
private[netflow] trait Template extends Flow[Template] {
  ...
  lazy val arrayMap: Array[String] =
    map.flatMap(b => Array(b._1, b._2.toString)).toArray
  ...
  lazy val isOptionTemplate: Boolean = flowSetId == 1 || flowSetId == 3

  def typeOffset(typeName: TemplateFields.Value): Int =
    map.get("offset_" + typeName.id) match {
      case Some(offset: Int) => offset
      case _                 => -1
    }

  def typeLen(typeName: TemplateFields.Value): Int =
    map.get("length_" + typeName.id) match {
      case Some(len: Int) => len
      case _              => 0
    }

  def key(): (InetSocketAddress, UUID) = (sender, id)
  def hasField(typeName: TemplateFields.Value): Boolean =
    map.contains("offset_" + typeName.id)

  import io.netflow.flows.cflow.TemplateFields._
  lazy val hasSrcAS: Boolean = hasField(SRC_AS)
  lazy val hasDstAS: Boolean = hasField(DST_AS)
  lazy val hasDirection: Boolean = hasField(DIRECTION)

  lazy val fields: Iterable[TemplateFields.Value] = map.keys
    .filter(_.startsWith("offset_"))
    .map(b => TemplateFields(b.replace("offset_", "").toInt))
  ...
  private val excludeFields = List(
    ...
    LAST_SWITCHED,
    TCP_FLAGS
    // SNMP_INPUT, SNMP_OUTPUT, SRC_MASK, DST_MASK
  )

  lazy val extraFields: Iterable[TemplateFields.Value] = map.keys
    .filter(_.startsWith("offset_"))
  ...
  lazy val json: String = Serialization.write {
    ("template" -> number) ~ ("fields" -> map)
  }
}

private[netflow] case class NetFlowV9Template(number: Int,
    ...
    extends Template {
  val versionNumber = 9
  lazy val id: UUID = UUIDs.timeBased()
}

private[netflow] object NetFlowV9Template
    extends TemplateMeta[NetFlowV9Template] {
  def apply(sender: InetSocketAddress,
            id: Int,
            packet: UUID,
            timestamp: DateTime,
  ...
  private def doLayer[T](
      f: NetFlowTemplateMeta[NetFlowV9Template] => Future[T]): Future[T] =
