...
object NetFlowV7Packet {
  ...
  /**
    ...
    */
  def apply(sender: InetSocketAddress, buf: ByteBuf): Try[NetFlowV7Packet] =
    Try[NetFlowV7Packet] {
  ...
  def apply(sender: InetSocketAddress,
            ...
            fpId: UUID,
            uptime: Long,
            timestamp: DateTime): Option[NetFlowV7] =
    Try[NetFlowV7] {
      NetFlowV7(
        UUIDs.timeBased(),
        sender,
  ...
  private def doLayer[T](
      f: FlowPacketMeta[NetFlowV7Packet] => Future[T]): Future[T] =
