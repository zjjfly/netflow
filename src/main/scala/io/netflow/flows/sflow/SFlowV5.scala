...
object SFlowV5Packet extends Logger {
  ...
  /**
    ...
    */
  def apply(sender: InetSocketAddress, buf: ByteBuf): Try[SFlowV5Packet] =
    Try[SFlowV5Packet] {
...
case class SFlowV5Packet(id: UUID,
...
case class SFlowV5(id: UUID,
                   sender: InetSocketAddress,
                   length: Int,
                   packet: UUID)
    extends Flow[SFlowV5] {
  def version = "sFlowV5 Flow"

  lazy val json = {
    ("flowVersion" -> version) ~ ("flowSender" -> "%s:%s".format(senderIP,
                                                                 senderPort))
  }.toString
}
